...
export const useControls = ({ setUpdating }: { setUpdating: (updating: boolean) => void }) => {
  ...
  // Load user's Klavis integrations from database using SWR
  useFetchUserKlavisServers(isKlavisEnabledInEnv);

  // Load user's LobeHub Skill connections using SWR
  ...
  // Get the set of all Klavis server type identifiers (for filtering builtinList)
  // Use KLAVIS_SERVER_TYPES instead of connected servers here, since we want to filter out all possible Klavis types
  const allKlavisTypeIdentifiers = useMemo(
    () => new Set(KLAVIS_SERVER_TYPES.map((type) => type.identifier)),
    [],
  );
  // Get the set of all skill identifiers (for filtering builtinList)
  ...
  // Connected items are sorted first
  ...
  // Distinguish between community plugins and custom plugins
  ...
  // Build LobeHub group children (includes Builtin Agent Skills, builtin tools, and LobeHub Skill/Klavis)
  const lobehubGroupChildren: ItemType[] = [
    // 1. Builtin Agent Skills
    ...builtinAgentSkillItems,
    // 2. Builtin tools
    ...builtinItems,
    // 3. LobeHub Skill and Klavis (as built-in skills)
    ...skillItems,
  ];
  ...
  // Installed tab items - only show installed plugins
  const installedPluginItems: ItemType[] = useMemo(() => {
    ...
    // Merge enabled LobeHub Skill and Klavis items (as built-in skills)
    ...
    // Build builtin tool group children (includes Builtin Agent Skills, builtin tools, and LobeHub Skill/Klavis)

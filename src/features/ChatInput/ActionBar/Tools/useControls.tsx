...
export const useControls = ({ setUpdating }: { setUpdating: (updating: boolean) => void }) => {
  ...
  // Use SWR to load user's Klavis integrations (from database)
  useFetchUserKlavisServers(isKlavisEnabledInEnv);

  // Use SWR to load user's LobeHub Skill connections
  ...
  // Get all Klavis server type identifier set (used to filter builtinList)
  // Use KLAVIS_SERVER_TYPES instead of connected servers, because we want to filter out all possible Klavis types
  const allKlavisTypeIdentifiers = useMemo(
    () => new Set(KLAVIS_SERVER_TYPES.map((type) => type.identifier)),
    [],
  );
  // Get all skill identifier set (used to filter builtinList)

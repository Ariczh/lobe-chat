  ...
  ({ identifier, label, server, serverName, agentId }) => {
    ...
    const handleConnect = async () => {
      ...
      try {
        ...
        if (newServer) {
          // Automatically enable plugin after installation (using identifier)
          const newPluginId = newServer.identifier;
          await togglePlugin(newPluginId);

          // If already authenticated, directly refresh tool list, skip OAuth
          if (newServer.isAuthenticated) {
            await refreshKlavisServerTools(newServer.identifier);
          } else if (newServer.oauthUrl) {
            // OAuth required, open OAuth window and monitor close
    ...
    return (
      <Flexbox
        ...
        onClick={(e) => {
          e.stopPropagation();
          // If connected, click the entire row to toggle state

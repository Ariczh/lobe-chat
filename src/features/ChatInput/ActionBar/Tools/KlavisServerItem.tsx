...
const KlavisServerItem = memo<KlavisServerItemProps>(
  ({ identifier, label, server, serverName, agentId }) => {
    ...
    /**
     * Start fallback polling (when window.closed is not accessible)
     */
    const startFallbackPolling = useCallback(
      (serverName: string) => {
        // Already polling, do not start again
        ...
        windowCheckIntervalRef.current = setInterval(() => {
          try {
            ...
            if (oauthWindow.closed) {
              // Window closed, clean up listeners and check authentication status
              if (windowCheckIntervalRef.current) {
                clearInterval(windowCheckIntervalRef.current);
                windowCheckIntervalRef.current = null;
              }
              ...
              // Start polling to check authentication status after window closes
              startFallbackPolling(serverName);
            }
          } catch {
            // COOP blocked access, fall back to polling
    ...
    const handleConnect = async () => {
      ...
      try {
        ...
        if (newServer) {
          // Auto-enable the plugin after installation (using identifier)
          const newPluginId = newServer.identifier;
          await togglePlugin(newPluginId);

          // If already authenticated, refresh tool list directly and skip OAuth
          if (newServer.isAuthenticated) {
            await refreshKlavisServerTools(newServer.identifier);
          } else if (newServer.oauthUrl) {
            // OAuth required, open OAuth window and listen for close
    ...
    return (
      <Flexbox
        ...
        onClick={(e) => {
          e.stopPropagation();
          // If connected, click entire row to toggle state

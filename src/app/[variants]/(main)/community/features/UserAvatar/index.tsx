...
const checkNeedsProfileSetup = (
  ...
  // If avatarUrl field is empty, profile setup is required
...
const UserAvatar = memo(() => {
  ...
  const handleSignIn = useCallback(async () => {
    setLoading(true);
    try {
      // Unified signIn call — shows a confirmation dialog first
      // In trustedClient mode, confirmation opens the ProfileSetupModal
      // In OIDC mode, confirmation proceeds with the OIDC flow
      await signIn();
    } catch {
  ...
  // Otherwise, show the login button when unauthenticated or when profile setup is needed
